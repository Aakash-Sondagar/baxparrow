  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[15px] border border-border bg-card">
      ...
      <div className="flex flex-1 flex-col px-4 pt-[15px] pb-[17px]">
        ...
        <button
          onClick={() => nav("/p/" + p.slug)}
          className="cursor-pointer border-none bg-none px-0 pt-0.5 pb-0 text-left font-display text-base font-bold text-ink"
        >
          {p.name}
        </button>
        ...
        <button
          onClick={() => add(p._id)}
          className="mt-3 w-full cursor-pointer rounded-[10px] border-none bg-ink py-[11px] text-sm font-semibold text-bg"
        >
          Add to cart
        </button>
      </div>
